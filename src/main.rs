...
fn gen_board() -> Vec<PipePair> {
    ...
    for i in 1..5 {
        ...
        out.push(PipePair {x: i * 2380 - 500, y1: height, y2: height + spacing});
    }
    out
}
...
fn main() -> Result<(),Error> {
    ...
    let default_pos = (0, 3825, 0, false);
    let pos: Arc<Mutex<(i32, i32, i32, bool)>> = Arc::new(Mutex::new(default_pos));
    ...
    loop {
        {
            ...
            if p.3 {
                ...
                if stop {
                    send_commands(&vec!["PA0,3825;", "PD;"].iter().map(|x| x.to_string()).collect(), &mut port);
                    *p = default_pos;
                }
            }
        }
