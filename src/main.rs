...
fn main() -> Result<(),Error> {
    ...
    send_commands(&board_cmds, &mut port);
    send_commands(&vec!["SP2;", "PA0,3825;", "PD;"].iter().map(|x| { Vec::from(x.clone().as_bytes()) }).collect(), &mut port);
